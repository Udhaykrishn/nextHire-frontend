...
export function RecruitersTable({ recruiters, total, page, totalPages, limit, currentStatus }: RecruitersTableProps) {
    const router = useRouter();
    const pathname = usePathname();
    const searchParams = useSearchParams();
    const { isLoading, confirm, openConfirmModal, closeConfirmModal, confirmBlockUnblock } = useRecruiterActions();
    const { selectedRecruiter, isModalOpen, openDetailsModal, setIsModalOpen } = useRecruiterDetails();
    ...
    const columns: Column<Recruiter>[] = [
            ...
            accessor: (recruiter) => (
                <div className="flex items-center gap-4">
                    <Avatar className="h-10 w-10 ring-2 ring-background">
                        <AvatarImage src={recruiter.profile_url?.url} alt={recruiter.name} />
        ...
        },
        {
            ...
            accessor: (recruiter) => (
                ...
                >
                    {recruiter.status === 'active' ? 'Active' : recruiter.status === 'blocked' ? 'Blocked' : recruiter.status}
                </Badge>
            ),
            className: "px-6 py-5",
        },
        {
            header: "Joined",
            accessor: (recruiter) => new Date(recruiter.createdAt).toISOString().split('T')[0],
    ...
    const actions: Action<Recruiter>[] = [
        ...
        {
            label: (recruiter) => isBlocked(recruiter.status) ? "Unblock Recruiter" : "Block Recruiter",
            icon: (recruiter) => isBlocked(recruiter.status)
                ? <ShieldCheck className="h-4 w-4 text-green-600" />
                : <ShieldAlert className="h-4 w-4 text-red-600" />,
            onClick: (recruiter) => openConfirmModal(recruiter),
            className: (recruiter) => isBlocked(recruiter.status) ? "" : "text-red-600",
        },
    ];

    const emptyMessage = currentStatus === 'pending'
        ...
        <>
            ...
            {confirm.recruiter && (
                <ConfirmModal
                    ...
                    title={isBlocked(confirm.recruiter.status) ? "Unblock Recruiter" : "Block Recruiter"}
                    description={`Are you sure you want to ${isBlocked(confirm.recruiter.status) ? "unblock" : "block"} "${confirm.recruiter.name}"?`}
                    confirmText={isBlocked(confirm.recruiter.status) ? "Unblock" : "Block"}
